...
/* NPN sensor output is pulled low when an object is detected. */
static void Send_SensorState(void)
{
  uint8_t payload[2];
  payload[0] = 0U; /* First X-axis sensor. */
  payload[1] = (HAL_GPIO_ReadPin(X_SENSOR_1_GPIO_Port, X_SENSOR_1_Pin) == GPIO_PIN_RESET) ? 1U : 0U;
